...
serve(async (req) => {
  ...
  try {
    const { campaign_id, recipient_id, phone, message } = await req.json();
    ...
    if (configError || !webhookConfig) {
      console.error("SMS webhook not configured:", configError);
      return new Response(
        JSON.stringify({ error: "SMS webhook not configured" }),
    ...
    if (!webhookResponse.ok) {
      ...
      return new Response(
        JSON.stringify({ error: "SMS delivery failed", details: errorText }),
        { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }
